...
function createTaskWithRetries(maxRetries: number, retryCount = 0): string {
...
describe('Retry Policy — Story 5.3', () => {
  ...
  it('exponential backoff: second retry ~10s', async () => {
    const taskId = createTaskWithRetries(3, 1); // already retried once
    const before = Date.now();
    const runId = await manager.startRun('agent-1', taskId);
    await manager.finishRun(runId, { exitCode: 1 });
  ...
  it('exponential backoff: third retry ~20s', async () => {
    const taskId = createTaskWithRetries(3, 2); // already retried twice
    const before = Date.now();
    const runId = await manager.startRun('agent-1', taskId);
    await manager.finishRun(runId, { exitCode: 1 });
  ...
  it('max retries exhausted — stays failed (does not reset to todo)', async () => {
    const taskId = createTaskWithRetries(2, 2); // maxRetries=2, already used both
    const runId = await manager.startRun('agent-1', taskId);
    await manager.finishRun(runId, { exitCode: 1 });
  ...
  it('task with max_retries=0 does not retry', async () => {
    const taskId = createTaskWithRetries(0, 0);
    const runId = await manager.startRun('agent-1', taskId);
    await manager.finishRun(runId, { exitCode: 1 });
  ...
  it('backoff capped at maxBackoffMs', async () => {
    ...
    const taskId = createTaskWithRetries(10, 5); // retry_count=5 → raw backoff = 5000*32 = 160s, capped at 8s
    const before = Date.now();
