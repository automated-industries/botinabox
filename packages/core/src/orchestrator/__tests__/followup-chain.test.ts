...
describe('Followup Chain — Story 5.4', () => {
  it('creates followup task when followup_agent_id is set', async () => {
    const taskRow = await db.insert('tasks', {
      title: 'Original Task',
      status: 'todo',
      priority: 5,
    ...
    const tasks = await db.query('tasks', { where: { assignee_id: 'agent-followup' } });
  ...
  it('chain_depth is incremented in followup task', async () => {
    ...
    const followup = await db.query('tasks', { where: { assignee_id: 'agent-next' } });
  ...
  it('no followup created on failure', async () => {
    ...
    const followups = await db.query('tasks', { where: { assignee_id: 'agent-followup' } });
